    ...
    <div class="container">
        <div class="row no-gutters-lg">
            ...
            <div class="col-lg-6 mb-5 mb-lg-0 mx-auto">
                <div class="row">                                       
                    <h2 class="section-title mb-3">{{ $power_item }}-修改類別</h2>
                    <form method="POST" action="{{ route('upload.type_update',$type->id) }}">
                        @csrf

                        <div class="mb-3">
                          <label for="order_by" class="form-label fw-bold">排序（可不填）</label>
                          <input type="number" class="form-control" name="order_by" value="{{ $type->order_by }}" id="order_by" placeholder="請輸入排序值">
                        </div>
                      
                        <div class="mb-3">
                          <label for="name" class="form-label fw-bold">名稱 <span class="text-danger">*</span></label>
                          <input type="text" class="form-control" name="name" value="{{ $type->name }}" id="name" required placeholder="請輸入名稱">
                        </div>
